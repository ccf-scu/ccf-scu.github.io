...
- 验证：22 项 Node 测试、Astro 检查、内容安全校验、33 页生产构建和构建产物校验通过；13 项常规端到端测试通过，并修复搜索结果数量写死导致的内容增长假失败。`CMS_LOCAL=1` 专项运行因本机防火墙阻止 Decap 本地后端连接而未完成，失败页面为 `ERR_NETWORK_ACCESS_DENIED`，不属于本次内容或构建逻辑回归；生产发布记录在完成后补充。
