...
- `/admin/` 先由 Decap 完成身份认证；只有 Decap 的用户状态确认已登录后，项目 React 壳层才提供六个一级路由、桌面侧栏、移动导航、页面工作区和图片中心；每个入口都可通过 hash 直接访问与刷新；
- `src/pages/admin/content-index.json.ts` 在构建期聚合已发布内容，只为后台页面提供列表和编排摘要，不包含草稿、凭据或私密信息；
- Decap 继续负责身份认证、集合读取、表单、原生 Markdown、预览和 Git editorial workflow；进入集合或具体编辑路由时，Decap 使用完整视口，自定义导航退化为 64px 顶部入口和覆盖式抽屉，不为固定侧栏预留宽度，避免开启双栏预览后挤压编辑区；从活动、公告、成员或荣誉编辑器返回集合根路由时，壳层按内容归属恢复对应新版工作区，显式“管理全部”入口不受影响；
- 固定版本 Decap 补丁只向壳层发出不含用户资料和 Token 的认证布尔事件，并关闭已确认无效的同步滚动控件；升级 Decap 时安装会要求重新审查补丁目标，并必须运行 CMS 专项浏览器测试；
