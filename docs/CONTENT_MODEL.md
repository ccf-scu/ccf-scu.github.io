...
`src/data/homepage.yml` 的 `featuredContent` 是首页公告、活动和荣誉的唯一选择来源。公告为 1–10 个稳定 ID；活动固定为 `academic`、`competition`、`tutoring`、`career` 四个类别槽位；荣誉固定 3 个且不可重复。引用不存在、隐藏、归档或活动类别不符时构建失败，不自动补位。
