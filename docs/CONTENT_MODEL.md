...
`src/data/homepage-featured.yml` 是首页公告、活动和荣誉的唯一选择来源，与 `homepage.yml` 的访客文案分开维护。公告为 1–10 个稳定 ID；活动固定为 `academic`、`competition`、`tutoring`、`career` 四个类别槽位；荣誉固定 3 个且不可重复。引用不存在、隐藏、归档或活动类别不符时构建失败，不自动补位。
...
固定长度数组由 Zod 和 CMS 的 `min`/`max` 同时限制，并关闭新增、删除和拖动，只允许修改原有项目。动画参数、章节编号、坐标和技术状态不进入 CMS。
