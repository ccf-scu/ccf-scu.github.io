...
`src/data/homepage-featured.yml` 只保存首页公告和荣誉引用：公告为 1–10 个稳定 ID，荣誉至少 1 个且不可重复，二者都按列表顺序展示。四个代表活动不再放在独立编排文件中，而是与方向文案一起保存在 `homepage.yml`。引用不存在、隐藏、归档或活动类别不符时构建失败，不自动补位。
